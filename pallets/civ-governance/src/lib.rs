...
pub mod pallet {
    ...
    pub enum Level { Parameter, Feature, CoreMechanism }

    #[derive(Clone, Encode, Decode, Eq, PartialEq, RuntimeDebug, TypeInfo, MaxEncodedLen)]
    pub enum Status { Active, Passed, ReadyToExecute, Executed, Rejected }
    ...
    pub trait Config: frame_system::Config {
        ...
        #[pallet::constant] type WBalance:     Get<u32>;
        #[pallet::constant] type WCirculation: Get<u32>;
        #[pallet::constant] type WReferral:    Get<u32>;
        #[pallet::constant] type WDonation:    Get<u32>;
    }
    ...
    #[pallet::storage] #[pallet::getter(fn verified_at)]
    pub type VerifiedAt<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AccountId, BlockNumberFor<T>, OptionQuery>;

    /// Aggregate governance score.
    #[pallet::storage] #[pallet::getter(fn score)]
    pub type Score<T: Config> =
    ...
    #[pallet::storage] #[pallet::getter(fn next_proposal)]
    pub type NextProposal<T: Config> = StorageValue<_, u32, ValueQuery>;

    #[pallet::storage] #[pallet::getter(fn proposals)]
    pub type Proposals<T: Config> =
        StorageMap<_, Twox64Concat, u32, Proposal<T>, OptionQuery>;

    #[pallet::storage]
    pub type Votes<T: Config> =
        StorageDoubleMap<_, Twox64Concat, u32, Blake2_128Concat, T::AccountId, bool, OptionQuery>;

    #[pallet::storage] #[pallet::getter(fn next_project)]
    pub type NextProject<T: Config> = StorageValue<_, u32, ValueQuery>;

    #[pallet::storage] #[pallet::getter(fn projects)]
    pub type Projects<T: Config> =
        StorageMap<_, Twox64Concat, u32, Project<T>, OptionQuery>;

    #[pallet::storage] #[pallet::getter(fn match_multiplier)]
    ...
    pub enum Event<T: Config> {
        GovernanceUnlocked,
        AgeRecorded   { who: T::AccountId },
        ScoreUpdated  { who: T::AccountId, score: u64 },
        Proposed      { id: u32, proposer: T::AccountId, level: Level },
        Voted         { id: u32, voter: T::AccountId, aye: bool, weight: u64 },
        StatusChanged { id: u32, status: Status },
        ProjectOpened { id: u32, target: BalanceOf<T> },
        Contributed   { id: u32, by: T::AccountId, amount: BalanceOf<T> },
        Matched       { id: u32, amount: BalanceOf<T> },
        Donated       { by: T::AccountId, amount: BalanceOf<T> },
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn donate(origin: OriginFor<T>, amount: BalanceOf<T>) -> DispatchResult {
            ...
            T::Currency::withdraw(&who, amount, WithdrawReasons::TRANSFER,
        ...
        pub fn propose(
            ...
            ensure!(T::Personhood::is_verified(&proposer), Error::<T>::NotVerified);
            ensure!(Self::is_unlocked(), Error::<T>::Locked);
            ensure!(Self::weight_of(&proposer) >= MIN_SCORE, Error::<T>::LowScore);

            let now      = frame_system::Pallet::<T>::block_number();
            let timelock = Self::timelock(&level);
            let id       = NextProposal::<T>::mutate(|n| { let v = *n; *n += 1; v });

            Proposals::<T>::insert(id, Proposal {
                id, proposer: proposer.clone(), level: level.clone(), hash,
                aye: 0, nay: 0, status: Status::Active,
                created: now,
                execute_after: now + VOTING_PERIOD.into() + timelock,
            });
            Self::deposit_event(Event::Proposed { id, proposer, level });
            Ok(())
        }
        ...
        pub fn vote(origin: OriginFor<T>, id: u32, aye: bool) -> DispatchResult {
            ...
            ensure!(Votes::<T>::get(id, &voter).is_none(), Error::<T>::AlreadyVoted);

            let now = frame_system::Pallet::<T>::block_number();
            let mut p = Proposals::<T>::get(id).ok_or(Error::<T>::NotFound)?;
            ensure!(p.status == Status::Active, Error::<T>::VotingClosed);
            ensure!(now < p.created + VOTING_PERIOD.into(), Error::<T>::VotingClosed);

            let w = Self::weight_of(&voter).max(1);
            if aye { p.aye = p.aye.saturating_add(w); }
            else   { p.nay = p.nay.saturating_add(w); }

            Votes::<T>::insert(id, &voter, aye);
            Proposals::<T>::insert(id, &p);
            Self::deposit_event(Event::Voted { id, voter, aye, weight: w });
            Ok(())
        }
        ...
        pub fn finalise(origin: OriginFor<T>, id: u32) -> DispatchResult {
            ...
            let passed = total > 0 &&
                p.aye.saturating_mul(100) >= total.saturating_mul(quorum);

            p.status = if passed { Status::Passed } else { Status::Rejected };
        ...
        pub fn mark_ready(origin: OriginFor<T>, id: u32) -> DispatchResult {
            ...
            Proposals::<T>::insert(id, &p);
            Self::deposit_event(Event::StatusChanged { id, status: Status::ReadyToExecute });
            Ok(())
        }
        ...
        pub fn open_project(
            ...
            if !self_fund.is_zero() {
                T::Currency::withdraw(&proposer, self_fund, WithdrawReasons::TRANSFER,
                    ExistenceRequirement::KeepAlive)
                    .map_err(|_| Error::<T>::InsufficientBalance)?;
            }
            let id = NextProject::<T>::mutate(|n| { let v = *n; *n += 1; v });
            Projects::<T>::insert(id, Project {
                id, proposer: proposer.clone(), hash, target,
                self_fund, community: BalanceOf::<T>::zero(),
                matched: BalanceOf::<T>::zero(), funded: false,
            });
        ...
        pub fn contribute(
            ...
            T::Currency::withdraw(&by, amount, WithdrawReasons::TRANSFER,
                ExistenceRequirement::KeepAlive)
                .map_err(|_| Error::<T>::InsufficientBalance)?;
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn recalc(who: &T::AccountId) {
            let (b, t, r, d) = Components::<T>::get(who);
            let s =
                (T::WBalance::get()     as u64).saturating_mul(Self::log2(b + 1))
            ...
            Self::deposit_event(Event::ScoreUpdated { who: who.clone(), score });
        }

        pub fn log2(n: u64) -> u64 {
            if n <= 1 { 0 } else { 63 - n.leading_zeros() as u64 }
        }
...
mod tests {
    ...
    impl PersonhoodProvider<u64> for MockId {
        fn is_verified(who: &u64) -> bool { *who <= 20 }
        fn verified_count() -> u64 { 10_001 }
    ...
    fn ext() -> sp_io::TestExternalities {
        ...
        pallet::GenesisConfig::<Test> { match_multiplier: Some(2), _ph: Default::default() }
    ...
    // ── Unlock conditions ──────────────────────────────────────────────
    #[test] fn locked_initially() {
        ext().execute_with(|| assert!(!CivGov::is_unlocked()));
    }
    #[test] fn unlocks_when_conditions_met() {
        ext().execute_with(|| {
            System::set_block_number(2_600_000);
    ...
    #[test] fn propose_fails_when_locked() {
        ext().execute_with(|| {
            give_score(1, 200);
            assert_noop!(
    ...
    // ── Age weight ─────────────────────────────────────────────────────
    #[test] fn no_age_record_zero_weight() {
        ext().execute_with(|| {
            give_score(1, 1000);
            assert_eq!(CivGov::weight_of(&1u64), 0);
        });
    }
    #[test] fn full_year_full_weight() {
        ext().execute_with(|| {
    ...
    // ── Proposal lifecycle ─────────────────────────────────────────────
    #[test] fn full_lifecycle_passes() {
        ext().execute_with(|| {
            unlock_and_age(&[1,2,3]);
            give_score(1, 300); give_score(2, 200); give_score(3, 100);

            assert_ok!(CivGov::propose(RuntimeOrigin::signed(1), Level::Parameter, [0u8;32]));
            ...
            assert_noop!(CivGov::vote(RuntimeOrigin::signed(1), 0, false), Error::<Test>::AlreadyVoted);
    ...
    #[test] fn proposal_rejected_below_quorum() {
        ext().execute_with(|| {
            unlock_and_age(&[1,2]);
            give_score(1, 100); give_score(2, 300);
            assert_ok!(CivGov::propose(RuntimeOrigin::signed(1), Level::Parameter, [1u8;32]));
            assert_ok!(CivGov::vote(RuntimeOrigin::signed(1), 0, true));
    ...
    #[test] fn double_fund_rejected() {
        ext().execute_with(|| {
            Balances::force_set_balance(RuntimeOrigin::root(), 1, 50_000).ok();
            Balances::force_set_balance(RuntimeOrigin::root(), 2, 50_000).ok();
            assert_ok!(CivGov::open_project(RuntimeOrigin::signed(1), [0u8;32], 500, 100));
            assert_ok!(CivGov::contribute(RuntimeOrigin::signed(2), 0, 400));
            assert_noop!(CivGov::contribute(RuntimeOrigin::signed(2), 0, 1), Error::<Test>::AlreadyFunded);
        });
    }
}
