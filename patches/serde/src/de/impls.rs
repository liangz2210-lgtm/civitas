...
macro_rules! num_as_copysign_self {
    ($ty:ident : $visit:ident) => {
            ...
            E: Error,
        {
            ...
            #[cfg(all(not(no_float_copysign), feature = "std", target_arch = "wasm32"))]
            {
                Ok(v as Self::Value)
            }

            #[cfg(all(not(no_float_copysign), feature = "std", not(target_arch = "wasm32")))]
            {
...
#[cfg(all(feature = "std", not(target_arch = "wasm32"), any(unix, windows)))]
variant_identifier! {
...
#[cfg(all(feature = "std", not(target_arch = "wasm32"), any(unix, windows)))]
#[cfg_attr(docsrs, doc(cfg(all(feature = "std", not(target_arch = "wasm32"), any(unix, windows)))))]
...
#[cfg(all(feature = "std", not(target_arch = "wasm32"), not(no_std_atomic)))]
...
#[cfg(all(feature = "std", not(target_arch = "wasm32"), not(no_std_atomic64)))]
atomic_impl! {
    AtomicI64 "64"
    AtomicU64 "64"
}
